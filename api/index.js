    ...
    try {
      ...
      if (AI_ENABLED) {
        await db2.insert(contactMessages).values({
          ...
          role: "owner",
          content: (msg.text ?? msg.caption ?? "(reply)").slice(0, 2e3)
        }).catch(() => {
        });
      }
      const ack = AI_ENABLED ? "\u2705 \u041E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u043E \u043A\u043B\u0438\u0435\u043D\u0442\u0443. \u0410\u0441\u0441\u0438\u0441\u0442\u0435\u043D\u0442 \u043F\u043E \u044D\u0442\u043E\u043C\u0443 \u0434\u0438\u0430\u043B\u043E\u0433\u0443 \u043E\u0442\u043A\u043B\u044E\u0447\u0451\u043D \u2014 \u0434\u0430\u043B\u044C\u0448\u0435 \u0432\u0440\u0443\u0447\u043D\u0443\u044E." : "\u2705 \u041E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u043E \u043A\u043B\u0438\u0435\u043D\u0442\u0443.";
...
async function listComps(db2) {
  return db2.select().from(valuationComps).orderBy(desc7(valuationComps.createdAt));
}
...
async function createPartner(db2, input) {
  ...
  const existing = await db2.select({ slug: partners.slug }).from(partners).where(sql8`(${partners.slug} = ${slug} OR ${partners.slug} LIKE ${slug + "-%"})`);
  if (existing.some((r) => r.slug === slug)) {
    let n = 2;
    const taken = new Set(existing.map((r) => r.slug));
    while (taken.has(`${slug}-${n}`)) n++;
...
async function updatePartner(db2, id, patch) {
  ...
  const [row] = await db2.update(partners).set(set).where(eq15(partners.id, id)).returning();
  return row ?? null;
}
...
async function createReferral(db2, input) {
  ...
  const [lead] = await db2.select({ id: leads.id }).from(leads).where(eq15(leads.id, leadId)).limit(1);
  if (!lead) throw new ReferralInputError(`lead ${leadId} not found`);
  const [partner] = await db2.select({ id: partners.id }).from(partners).where(eq15(partners.id, partnerId)).limit(1);
...
async function updateReferral(db2, id, patch) {
  const [current] = await db2.select().from(referrals).where(eq15(referrals.id, id)).limit(1);
  ...
  const [row] = await db2.update(referrals).set(set).where(eq15(referrals.id, id)).returning();
  return row ?? null;
}
async function listReferrals(db2) {
  return db2.select({
    ...
    leadName: leads.name
  }).from(referrals).leftJoin(partners, eq15(referrals.partnerId, partners.id)).leftJoin(leads, eq15(referrals.leadId, leads.id)).orderBy(desc8(referrals.createdAt));
}
