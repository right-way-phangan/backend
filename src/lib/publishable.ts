...
/**
 * Контакты в описании. Каждый паттерн ЗАМЕНЯЕТСЯ на плейсхолдер, а не режет
 * строку целиком: на пути записи удаление строки уносило вместе с телефоном и
 * цену, и характеристики — необратимо, потому что в БД правленый текст и есть
 * оригинал.
 *
 * Квантификаторы везде ограничены сверху. Прежняя почтовая альтернатива
 * `[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}` давала катастрофический бэктрекинг:
 * на 64k символов описания — десятки секунд в обработчике POST /objects.
 */
...
/** Денежный контекст: рядом с суммой цифры — это цена, а не телефон. */
const MONEY_NEAR = /(?:thb|฿|บาท|usd|\$|eur|€|rub|₽|млн|млрд|million|m\b|k\b|бат)/i;

/** Верхняя граница текста, который вообще прогоняется через регулярки. */
const MAX_SCAN_CHARS = 20_000;

// Буква любой из рабочих письменностей: `\b` в JS считается по ASCII и на
// границе с кириллицей не срабатывает вовсе.
...
const DEED_WORD = `(?<!${LETTER})(chanote|чанот[а-яё]*|титул[а-яё]*|title\\s*deed|nor\\s*sor\\s*3(?:\\s*kor)?|ns3k?)(?!${LETTER})`;
// Индикатор номера. Кириллическую ветку раньше убивал `\b`: ни перед «ч», ни
// после «т» ASCII-границы слова не возникает, и альтернатива была недостижима —
// «Чанот № 13681» не редактировался и не блокировался, в отличие от английского.
const DEED_INDICATOR = "(?:no\\.?|number|№|#|deed\\s*no\\.?|เลขที่)";
/**
 * Слово ПЕРЕД числом, после которого четырёхзначное число — не номер документа,
 * а год или порядковый номер лота. Прежняя проверка смотрела только вправо,
 * поэтому «Chanote issued in 2019», «sold in 2018» и «Chanote land plot 2400»
 * блокировали живой листинг во все каналы, а причина в UI выглядела как
 * обвинение в утечке.
 */
const NOT_AFTER_CONTEXT =
  `(?<!(?<!${LETTER})(?:issued|sold|built|registered|since|from|in|of|plot|lot|unit|parcel|` +
  `выдан[а-яё]{0,3}|продан[а-яё]{0,3}|построен[а-яё]{0,3}|зарегистрирован[а-яё]{0,3}|` +
  `участок|участке|лот|лоте|юнит|юните|от|в)\\s{1,3})`;
const REDACT_DEED = new RegExp(
...
const HARD_CONFIDENTIAL: Array<{ re: RegExp; label: string }> = [
  ...
  // площадью/градусами всегда идёт единица («Chanote: 1600 m²», «360 degrees»).
  {
    ...
    re: new RegExp(
      `${DEED_WORD}[^.\\n]{0,20}?${NOT_AFTER_CONTEXT}(?<![\\d,.])\\d{4,6}(?![\\d]|[.,]\\d)` +
        `(?!\\s*(?:m²|m2|sqm|sq\\.?\\s*m|square\\s*met|rai|ngan|wah|degrees|°|рай|нган|ва|м²|кв|` +
        `thb|฿|บาท|usd|\\$|eur|rub|₽|бат|год|г\\.|year))`,
      "i",
    ),
...
export function redactConfidential(raw: string, warnings: string[] = []): string {
  if (raw.length > MAX_SCAN_CHARS) {
    // Описание такой длины — не листинг, а вставленный документ. Обрезаем до
    // сканируемого объёма, чтобы регулярки не превращались в DoS-примитив.
  ...
  return s
    .split(/\r?\n/)
    .map((ln) => {
      // Строка с денежным маркером и без явного «+телефона» — это цена: не трогаем.
      const moneyLine = MONEY_NEAR.test(ln);
      let out = ln;
      for (const re of CONTACT_PATTERNS) {
        re.lastIndex = 0;
        out = out.replace(re, (hit) => {
          if (moneyLine && /^[\d\s.,-]+$/.test(hit)) return hit; // сумма, а не контакт
          warnings.push("из описания скрыт контакт/телефон");
          // Пустая строка, а не словесный плейсхолдер: любой текст здесь попадает
          // в описание обоих языков, а кириллица в EN-ветке роняет весь пост.
          return "";
        });
      }
      return out;
