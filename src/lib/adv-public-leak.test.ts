...
test("АТАКА 1: телефон и комиссия из description не доезжают до публичного /objects", async () => {
  ...
  // Текст вокруг контакта СОХРАНЁН: редакция вырезает сам контакт, а не строку
  // целиком (удаление строки уносило вместе с телефоном цену и характеристики —
  // необратимо, потому что в БД правленый текст и есть оригинал).
  assert.match(desc, /Khun Somchai/);
});
